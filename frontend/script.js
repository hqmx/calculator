...
function initializeApp() {
    ...
    // Expose state and function wrappers immediately for testing
    // Functions will be defined later, but wrappers allow early access
    window.state = state;
    window.selectFormat = (...args) => selectFormat(...args);
    window.openConversionModal = (...args) => openConversionModal(...args);
    window.removeFile = (...args) => removeFile(...args);
    window.startBatchConversion = (...args) => startBatchConversion(...args);
    console.log('✅ window.state and function wrappers exposed early for testing');
    ...
    // 변환 지원 여부 캐시 (API 호출 최소화)
    const conversionSupportCache = new Map();

    /**
     * 백엔드 API로 변환 지원 여부 확인
     * @param {string} from - 입력 형식
     * @param {string} to - 출력 형식
     * @returns {Promise<boolean>} 지원 여부
     */
    async function checkConversionSupported(from, to) {
        const cacheKey = `${from}-${to}`;

        // 캐시 확인
        if (conversionSupportCache.has(cacheKey)) {
            return conversionSupportCache.get(cacheKey);
        }

        try {
            const response = await fetch(`${API_BASE_URL}/conversions/check?from=${from}&to=${to}`);
            if (!response.ok) {
                console.error(`[API] Conversion check failed: ${response.status}`);
                return false; // 에러 시 비활성화 처리
            }
            ...
            // 캐시 저장
            conversionSupportCache.set(cacheKey, supported);

            return supported;
        } catch (error) {
            console.error(`[API] Error checking conversion support (${from} → ${to}):`, error);
            return false; // 에러 시 비활성화 처리
        }
    }
    ...
    const FORMATS = {
        video: ['mp4', 'avi', 'mov', 'mkv', 'webm', 'flv', 'wmv', '3gp', 'm4v', 'mpeg', 'ogv'],
        audio: ['mp3', 'wav', 'flac', 'aac', 'ogg', 'm4a', 'wma', 'opus', 'aiff', 'ac3'],
        image: ['jpg', 'jpeg', 'png', 'gif', 'webp', 'heic', 'avif', 'svg', 'bmp', 'tiff', 'ico', 'psd', 'tga', 'cr2', 'cr3', 'nef', 'arw', 'dng', 'raf', 'orf', 'rw2'],
    ...
    const CROSS_CATEGORY_COMPATIBILITY = {
        ...
        document: {
            ...
            formatRestrictions: {
                // 문서 → 문서: 오직 → PDF만 가능 (LibreOffice 제약)
                document: {
                    sourceFormats: ['doc', 'docx', 'xlsx', 'xls', 'pptx', 'ppt', 'txt', 'rtf', 'odt', 'ods', 'odp'], // Office 문서만
                    targetFormats: ['pdf'] // PDF로만 변환 가능
                },
                // PDF → 이미지만 가능 (다른 문서는 이미지로 변환 불가)
                image: {
                    sourceFormats: ['pdf'], // PDF만 이미지로 변환 가능
                    targetFormats: ['jpg', 'jpeg', 'png', 'webp', 'heic', 'gif', 'svg', 'bmp', 'tiff', 'ico', 'psd', 'tga', 'avif'] // 모든 이미지 형식 지원 (2-step 변환)
                }
            }
        },
    ...
    const ADVANCED_SETTINGS = {
        ...
        image: {
            ...
            resize: { label: 'Resize', type: 'select', options: ['original', '50%', '75%'], default: 'original' },
    ...
    if (hamburgerMenu && mobileMenuOverlay) {
        ...
        document.querySelectorAll('.mobile-nav-link').forEach(link => {
            link.addEventListener('click', (e) => {
                ...
                // href가 실제 URL인 경우 (/, /how-to-use, /faq, /api 등) 기본 동작 허용
                if (href && !href.startsWith('#')) {
                    closeMobileMenu();
                    return; // 기본 동작(페이지 이동) 허용
                }

                // data-section이 있는 경우만 기본 동작 막고 스크롤 처리
                if (section) {
                    e.preventDefault();
    ...
    // 제거: collapsed 상태 처리 로직이 파일 선택을 방해함
    // uploadZone의 collapsed 상태와 관계없이 파일 선택이 항상 작동하도록 수정
    ...
    function closeModal() {
        ...
        // NOTE: state.batchFiles는 배치 변환 완료 후에 정리됨 (중복 다운로드 방지)
    ...
    function filterAvailableCategories(fileCategory) {
        ...
        if (state.currentFileIndex >= 0) {
            sourceExtension = state.files[state.currentFileIndex].extension.toLowerCase();
        } else if (state.batchFiles && state.batchFiles.length > 0) {
            sourceExtension = state.batchFiles[0].extension.toLowerCase();
        }
        ...
        dom.formatCategories.forEach(cat => {
            ...
            let isAllowed = allowed.includes(category);

            // 추가 제한: 형식별 세부 제한 확인
            if (isAllowed && compatibility && compatibility.formatRestrictions && compatibility.formatRestrictions[category]) {
                const restrictions = compatibility.formatRestrictions[category];

                // 객체 형식 제한: sourceFormats 확인
                if (restrictions.sourceFormats && Array.isArray(restrictions.sourceFormats)) {
                    // sourceExtension이 허용된 형식에 포함되어 있는지 확인
                    isAllowed = restrictions.sourceFormats.includes(sourceExtension);
                }
            }
    ...
    async function handleCategorySelect(category, recommendedFormats = []) {
        // Update active category
        dom.formatCategories.forEach(cat => {
            cat.classList.toggle('active', cat.dataset.category === category);
        });

        // 현재 파일 정보 가져오기 (변환 지원 여부 확인용)
        ...
        // 로딩 표시 제거 - 빠른 렌더링을 위해 즉시 버튼 표시
        // formatGrid.innerHTML = '<div class="format-loading">Checking supported formats...</div>';
        // dom.formatOptions.appendChild(formatGrid);

        // 모든 형식 버튼 생성 및 지원 여부 확인 (병렬 처리)
        const formatChecks = formats.map(async (format) => {
            ...
            if (sourceExtension) {
                isSupported = await checkConversionSupported(sourceExtension, format);
            }
            ...
            if (!isSupported) {
                ...
                option.title = `Conversion ${sourceExtension.toUpperCase()} → ${format.toUpperCase()} is not supported`;
            } else {
                // 지원 형식: 클릭 이벤트 추가
        ...
        // 모든 API 호출 완료 대기
        const formatOptions = await Promise.all(formatChecks);

        // 버튼 추가 (로딩 메시지 없이 즉시 표시)
        formatOptions.forEach(option => formatGrid.appendChild(option));
        dom.formatOptions.appendChild(formatGrid);
    }
    ...
    function startBatchConversion() {
        ...
        // state에서 선택된 형식 가져오기 (DOM이 아닌 state 사용)
        let outputFormat = state.selectedFormat;
        let selectedCategory = state.selectedCategory;
        console.log('선택된 형식 (state):', outputFormat);

        // 형식이 선택되지 않았으면 첫 번째 형식을 자동 선택
        if (!outputFormat) {
            console.log('⚠️ 선택된 형식이 없음, 첫 번째 형식 자동 선택');
            const firstFormat = document.querySelector('.format-badge:not(.disabled)');
            if (firstFormat) {
                firstFormat.click(); // 첫 번째 형식 자동 선택
                outputFormat = firstFormat.dataset.format;
                selectedCategory = firstFormat.classList.contains('image') ? 'image' :
                                   firstFormat.classList.contains('video') ? 'video' :
                                   firstFormat.classList.contains('audio') ? 'audio' : 'document';
                showToast('Auto-selected first format. Click again to choose different format.', 'info');
            } else {
                console.error('❌ 사용 가능한 형식이 없음');
                showToast('No output formats available', 'error');
                return;
            }
        }
    ...
    async function startBatchFileConversions(files) {
        ...
        // Phase 3: Use BatchConversionManager if available (Advanced Features)
        if (window.batchConversionManager) {
            console.log('✅ Using Phase 3 BatchConversionManager');
            try {
                await window.batchConversionManager.startBatch(files);
                return;
            } catch (error) {
                console.error('❌ BatchConversionManager error, falling back to legacy:', error);
                // Continue to fallback logic
            }
        }

        // Fallback: Original sequential conversion logic
        console.log('⚠️ BatchConversionManager not available or failed, using fallback');
        ...
        console.log('✅ 배치 변환 완료, state.batchFiles 정리됨');
    }
    ...
    const SERVER_SIDE_FORMATS = {
        ...
        // RAW 사진 형식 (복잡한 디코딩 필요) + PSD, TGA
        raw: ['cr2', 'cr3', 'nef', 'arw', 'dng', 'orf', 'rw2', 'raf'],
        exotic: ['psd', 'tga', 'ico'],  // ImageMagick 전용 형식
        // Office 문서 형식 (복잡한 변환)
    ...
    function needsServerSideConversion(fileObj) {
        ...
        // 4. Exotic 이미지 형식 (PSD, TGA, ICO - ImageMagick 전용)
        if (SERVER_SIDE_FORMATS.exotic.includes(inputExt) ||
            SERVER_SIDE_FORMATS.exotic.includes(outputExt)) {
            return true;
        }

        // 5. Office 문서 형식 (복잡한 변환)
    ...
    // Note: Functions already exposed via wrappers at the beginning of initializeApp()
    // No need to expose again here
    ...
    function initializeGoogleDrive() {
        ...
        const CLIENT_ID = localStorage.getItem('googleDriveClientId') || '280988173097-ffdh6ft1kujjcn5kp9md1p6mso17cvpj.apps.googleusercontent.com';
        ...
        function gapiLoaded() {
            ...
            // Load both client and picker
            gapi.load('client:picker', async () => {
                console.log('🟢 [Google Drive] gapi.client 및 Picker 로드 완료');
                const initConfig = {
        ...
        function maybeEnableButtons() {
            console.log('🟢 [Google Drive] 버튼 활성화 확인... gapiInited:', gapiInited, 'gisInited:', gisInited, 'pickerInited:', pickerInited);
        ...
        async function showGoogleDrivePicker() {
            try {
                ...
                console.log('🟢 [Google Drive] Google Picker 생성 중...');

                // Create and render the Picker
                const picker = new google.picker.PickerBuilder()
                    .addView(google.picker.ViewId.DOCS)  // All document types
                    .addView(google.picker.ViewId.DOCS_IMAGES)  // Images
                    .addView(google.picker.ViewId.DOCS_VIDEOS)  // Videos
                ...
                console.log('🟢 [Google Drive] ✅ Google Picker 표시됨');
            } catch (error) {
                console.error('🔴 [Google Drive] Picker 생성 실패:', error);
        ...
        function pickerCallback(data) {
            if (data.action === google.picker.Action.PICKED) {
                console.log('🟢 [Google Drive] 파일 선택됨:', data.docs.length, '개');

                // Process each selected file
                data.docs.forEach(file => {
                    console.log('🟢 [Google Drive] 처리 중:', file.name, '-', file.mimeType);

                    // Skip Google Workspace files (they can't be downloaded directly)
                    if (file.mimeType && file.mimeType.startsWith('application/vnd.google-apps')) {
                        console.log('🟡 [Google Drive] Google Workspace 파일 제외:', file.name);
                        showToast(`Cannot convert Google Workspace files (${file.name}). Please upload regular files.`, 'warning');
                        return;
                    }

                    downloadFromGoogleDrive(file.id, file.name, file.mimeType);
                });
            } else if (data.action === google.picker.Action.CANCEL) {
                console.log('🟢 [Google Drive] 사용자가 취소함');
            }
        }

        async function downloadFromGoogleDrive(fileId, fileName, mimeType) {
            try {
                console.log('🟢 [Google Drive] 파일 다운로드 시작:', fileName, '- Type:', mimeType);
                ...
                console.log('🟢 [Google Drive] OAuth 토큰 확인됨');

                // Download file using fetch with OAuth token
                ...
                if (!response.ok) {
                    let errorMsg = `Download failed: ${response.status} ${response.statusText}`;
                    if (response.status === 403) {
                        errorMsg += ' - This file may be restricted or you may not have permission to download it.';
                    }
                ...
                const fileObj = new File([blob], fileName, { type: mimeType || blob.type || 'application/octet-stream' });
                addFiles([fileObj]);
                showToast(`${fileName} loaded from Google Drive`, 'success');
            } catch (error) {
                console.error('🔴 [Google Drive] 다운로드 실패:', error);
                const errorMessage = error.message || 'Failed to download file from Google Drive';
                    ...
                    <div class="modal-body">
                        ...
                        <details style="margin-bottom: 1.5rem; padding: 1rem; background: #f8f9fa; border-radius: 8px;">
                            ...
                            <ol style="margin-left: 1.5rem; line-height: 1.8;">
                                ...
                                <li>Authorized JavaScript origins에 <code>https://hqmx.net</code> 추가</li>
                            </ol>
                        </details>
...
function showAdsOnDownloadComplete() {
    // ExoClick Vignette 로드 (다운로드 완료 시 1회만)
    if (!window.exoClickVignetteLoaded) {
        window.exoClickVignetteLoaded = true;
        var vignetteScript = document.createElement('script');
        vignetteScript.type = 'text/javascript';
        vignetteScript.setAttribute('data-zone', '10017255');
        vignetteScript.src = 'https://groleegni.net/vignette.min.js';
        document.body.appendChild(vignetteScript);
        console.log('[Ads] ExoClick Vignette 광고 로드 (다운로드 완료)');
    }
}
