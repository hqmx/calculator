...
class ConverterEngine {
    ...
    async initFFmpeg(onProgress) {
        ...
        try {
            onProgress?.(10, 'Loading FFmpeg library...');
            ...
            onProgress?.(30, 'Loading FFmpeg core...');
    ...
    async convertWithServer(file, outputFormat, options = {}, onProgress) {
        try {
            ...
            onProgress?.(20, 'Waiting for conversion...');
            ...
            onProgress?.(85, 'Downloading converted file...');
            ...
            onProgress?.(100, 'Server conversion completed!');
            return blob;
        ...
        return new Promise((resolve, reject) => {
            ...
            img.onload = async () => {
                try {
                    ...
                    // 더 안전한 변환 방식
                    try {
                        console.log('Canvas Starting conversion...');

                        // 1단계: toDataURL 시도
                        let dataURL;

                        if (mimeType === 'image/jpeg') {
                            // JPEG는 quality 0.1~0.98 범위로 제한 (1.0은 피함)
                            const safeQuality = Math.min(Math.max(quality || 0.8, 0.1), 0.98);
    ...
    async extractVideoThumbnail(videoElement, outputFormat, onProgress) {
        onProgress?.(30, 'Extracting thumbnail...');
        ...
        return new Promise((resolve, reject) => {
            videoElement.onseeked = () => {
                try {
                    ...
                    canvas.toBlob(blob => {
                        if (blob) {
                            onProgress?.(100, 'Thumbnail extracted!');
                            resolve(blob);
                        } else {
    ...
    async convertVideoFormat(videoElement, outputFormat, options, onProgress) {
        onProgress?.(30, 'Processing video stream...');
    ...
    async convertVideoToGif(videoElement, options = {}, onProgress) {
        onProgress?.(10, 'Preparing GIF conversion...');
    ...
    async convertGifToVideo(file, outputFormat, options = {}, onProgress) {
        ...
        try {
            ...
            onProgress?.(20, 'Preparing GIF file...');

            // 입력 GIF 파일 쓰기
            this.ffmpeg.FS('writeFile', inputName, await this.fetchFile(file));

            onProgress?.(30, 'Starting GIF to video conversion...');
            ...
            onProgress?.(90, 'Extracting converted video...');
            ...
            onProgress?.(100, 'GIF to video conversion completed!');
            return blob;
    ...
    async pdfToImage(pdfFile, imageFormat, options = {}, onProgress) {
        ...
        onProgress?.(30, 'Rendering PDF pages...');
    ...
    async textToPdf(textFile, options = {}, onProgress) {
        ...
        onProgress?.(30, 'Preparing PDF creation...');
    ...
    async docToPdf(docFile, options = {}, onProgress) {
        onProgress?.(10, 'Sending DOC file to server...');
    ...
    async excelToPdf(excelFile, options = {}, onProgress) {
        try {
            onProgress?.(10, 'Processing Excel...');

            // SheetJS 및 jsPDF 로드
            await Promise.all([this.loadSheetJS(), this.loadJsPdf()]);

            onProgress?.(20, 'Parsing Excel...');
    ...
    async powerPointToPdf(pptFile, options = {}, onProgress) {
        onProgress?.(10, 'Sending PowerPoint file to server...');
    ...
    async convertOnServer(file, outputFormat, onProgress) {
        ...
        try {
            onProgress?.(20, 'Preparing server conversion...');
            ...
            onProgress?.(80, 'Downloading converted file...');
    ...
    /**
     * 변환 중단 (FFmpeg 프로세스 종료)
     */
    async abort() {
        console.log('[ConverterEngine] abort() 호출됨');

        if (this.ffmpeg && this.ffmpegLoaded) {
            try {
                // FFmpeg.wasm 0.12.x의 terminate() 메서드 호출
                await this.ffmpeg.terminate();
                console.log('[ConverterEngine] FFmpeg 프로세스 종료 완료');
                ...
                console.error('[ConverterEngine] FFmpeg 종료 실패:', error);
                // 에러가 발생해도 상태는 리셋
                this.ffmpegLoaded = false;
                this.ffmpegLoading = false;
                this.ffmpeg = null;
            ...
            console.log('[ConverterEngine] FFmpeg가 로드되지 않았거나 이미 종료됨');
            return false;
        }
    }
